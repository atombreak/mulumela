...
// POST /api/invitations - Create a new invitation and optionally send it
  ...
  try {
    ...
    if (sendImmediately && invitationMethods.length > 0) {
      if (invitationMethods.includes('email') && !email) {
        return NextResponse.json(
          { error: 'Email is required when sending via email' },
          { status: 400 }
        );
      }
      if ((invitationMethods.includes('sms') || invitationMethods.includes('whatsapp')) && !phone) {
        return NextResponse.json(
          { error: 'Phone number is required when sending via SMS or WhatsApp' },
          { status: 400 }
        );
      }
    }
    ...
    if (sendImmediately && invitationMethods.length > 0 && event) {
